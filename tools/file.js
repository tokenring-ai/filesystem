...
export async function execute({
 ...
 chatService.infoLine(`[fileManager] Performing ${action} operation via ${fileSystem.name}: ${filePath}`);
 ...
 try {
  switch (action) {
   ...
   case 'replace': {
    ...
    if (permissions) {
     const numericPermissions = typeof permissions === 'string' ? parseInt(permissions, 8) : permissions;
   ...
   case 'adjust': {
    ...
    if (!adjustExists) {
     chatService.errorLine(`Cannot modify permissions for file ${filePath}: file not found.`);
...
export const description = "Creating, updating, deleting, and rename files, and adjust file properties.";

export const parameters = z.object({
 path: z.string().describe("Path of the file to operate on. Will be created if it doesn't exist."),
 action: z.enum(["create", "replace", "delete", "rename", "adjust"]).describe("The action to perform on the file."),
 content: z.string().describe("Full text content when creating or updating a file. ALWAYS include the ENTIRE file contents. It is an absolutely critical failure to not output the entire file.").optional(),
 permissions: z.string().describe("File permissions string (e.g., '644' or '0o644') to set. Parsed as octal. Default: '644' (if not provided, no changes are made by default for 'adjust' action, sensible defaults for 'create').").optional(),
 owner: z.string().describe("Owner of the file. Default: current user.").optional(),
 toPath: z.string().describe("New path/name for the file when renaming. Will create missing directories if needed.").optional()
});
