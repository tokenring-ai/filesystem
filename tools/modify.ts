...
export async function execute(
  ...
  switch (action) {
    ...
    case "append": {
      if (!content) {
        throw new Error(`[${name}] 'content' is required for ${action} actions`);
      }

      // Check existence if needed
      const fileExists = await fileSystem.exists(filePath);
      if (action === "write" && fail_if_exists && fileExists) {
        throw new Error(`[${name}] File ${filePath} already exists and 'fail_if_exists' is true`);
      }
      ...
      if (success && !fileExists) { // Only default for truly new files
    ...
    case "delete": {
      const fileExists = await fileSystem.exists(filePath);
      if (!fileExists && check_exists) { // Default to error if missing
    ...
    case "rename": {
      if (!toPath) {
        throw new Error(`[${name}] 'toPath' parameter is required for rename action`);
      }
    ...
    case "adjust": {
      ...
      if (!fileExists && check_exists) {
        throw new Error(`[${name}] Cannot adjust file ${filePath}: file not found`);
      }
      if (!fileExists) {
        return "File not found, no action taken";
      }
    ...
    default:
      throw new Error(`[${name}] Unsupported action '${action}'. Supported actions: write, append, delete, rename, adjust`);
  }
}
...
export const inputSchema = z.object({
  path: z
    .string()
    .describe("Relative path of the file to operate on (e.g., 'src/main.ts' or '/docs/design.md'). Starts from virtual FS root. Required."),
  action: z
    .enum(["write", "append", "delete", "rename", "adjust"])
    .describe("Action to perform: 'write' creates/overwrites with full content; 'append' adds to end; 'delete' removes; 'rename' moves/renames; 'adjust' changes properties."),
  content: z
    .string()
    .describe(
  ...
  is_base64: z
    .boolean()
    .describe("If true, 'content' is base64-encoded binary data (decoded before writing). Default: false (treat as UTF-8 text).")
    .optional(),
  fail_if_exists: z
    .boolean()
  ...
  check_exists: z
    .boolean()
    .describe("For 'delete', 'rename', 'adjust': If true (default), error if file not found. If false, return 'no action taken' silently.")
    .optional(),
});