...
export async function execute({
 ...
 try {
  ...
  const normalizeWhitespace = (line) => line.trim();
...
export const description = "Patches a file by replacing content between two specific lines that match exactly (ignoring whitespace).";

export const parameters = z.object({
 file: z.string().describe("Path to the file to patch, relative to the source directory."),
 fromLine: z.string().describe("A line of text that must match exactly to mark the beginning of the content to replace."),
 toLine: z.string().describe("A line of text that must match exactly to mark the end of the content to replace."),
 contents: z.string().describe("The content that will replace everything from fromLine to toLine (inclusive)."),
});
