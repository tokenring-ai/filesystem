...
export async function execute(
	...
	if (
		...
		throw new Error(
			"returnType must be one of: 'names', 'content', or 'matches'",
		);
	}
		...
		for (const filePattern of files) {
			try {
				...
				chatService.errorLine(
					`Error resolving pattern ${filePattern}: ${err.message}`,
				);
			}
		}
...
async function fileSearch(
	...
	if (Array.isArray(searchString)) {
		chatService.infoLine(
			`[fileManager] Searching for multiple patterns: ${JSON.stringify(searchString)}`,
		);
	} else {
