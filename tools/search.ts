...
export async function execute(
  ...
  if (matchType !== "substring" && matchType !== "whole-word" && matchType !== "regex") {
    throw new Error(`[${name}] matchType must be one of: 'substring', 'whole-word', or 'regex'`);
  }
...
async function fileSearch(
  ...
  try {
    ...
    if (results.length > 50 && returnType === "matches") {
      ...
      result.files = uniqueFiles.map(file => ({file, exists: true, content: null}));
...
async function searchInFiles(
  ...
  for (const fileResult of fileResults) {
    ...
    const normalizedLines = caseSensitive ? lines : lines.map(line => line.toLowerCase());
...
export const inputSchema = z
  .object({
    ...
    caseSensitive: z
      .boolean()
      .describe(
        "Whether searches are case-sensitive. Default: true.",
      )
      .optional(),
    matchType: z
