...
export const description = "/file [action] [files...] - Manage files in the chat session (select, add, remove, list, clear, defaults).";

async function selectFiles(filesystem, chatService, humanInterfaceService) {
 try {
  const selectedFiles = await humanInterfaceService.askForFileSelection(filesystem, {
   initialSelection: Array.from(filesystem.getFilesInChat())
  });

  filesystem.setFilesInChat(selectedFiles);
  chatService.systemLine(`Selected ${selectedFiles.length} files for chat session`);
 } catch (error) {
...
async function addFiles(filesystem, chatService, humanInterfaceService, filesToAdd) {
  let addedCount = 0;
  ...
  if (addedCount > 0) {
    chatService.systemLine(`Successfully added ${addedCount} file(s) to the chat session.`);
  }
}

async function removeFiles(filesystem, chatService, humanInterfaceService, filesToRemove) {
  let removedCount = 0;
  ...
  if (removedCount > 0) {
    chatService.systemLine(`Successfully removed ${removedCount} file(s) from the chat session.`);
  }
}

async function listFiles(filesystem, chatService) {
  const filesInChat = Array.from(await filesystem.getFilesInChat());
  ...
  filesInChat.forEach((file, index) => {
    chatService.systemLine(`  ${index + 1}. ${file}`);
  });
}

async function clearFiles(filesystem, chatService) {
 await filesystem.setFilesInChat([]);
 chatService.systemLine("Cleared all files from the chat session.");
}

async function defaultFiles(filesystem, chatService) {
 const defaultFiles = await filesystem.getDefaultFiles();
...
export async function execute(remainder, registry) {
  ...
  switch (action) {
    ...
    case 'add':
      await addFiles(filesystem, chatService, humanInterfaceService, actionArgs);
      break;

    case 'remove':
    case 'rm':
      await removeFiles(filesystem, chatService, humanInterfaceService, actionArgs);
      break;
    ...
    default:
      ...
      chatService.systemLine("  /file select             - Interactive file selection");
      chatService.systemLine("  /file add [files...]     - Add files to chat session");
      chatService.systemLine("  /file remove [files...]  - Remove files from chat session");
      chatService.systemLine("  /file list               - List files in chat session");
      chatService.systemLine("  /file clear              - Remove all files from chat session");
      chatService.systemLine("  /file default            - Reset selected files to your config default");
      chatService.systemLine("");
      chatService.systemLine("Examples:");
      chatService.systemLine("  /file select " +
       "" +
       "            - Interactive file selection");
      chatService.systemLine("  /file add src/index.js   - Add specific file");
      chatService.systemLine("  /file remove index.js    - Remove specific files");
