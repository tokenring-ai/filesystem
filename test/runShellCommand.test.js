...
describe('runShellCommand Integration Tests', () => {
	...
	describe('Basic Command Execution', () => {
		it('should execute a basic command successfully', async () => {
			...
			expect(mockFileSystem.executeCommand).toHaveBeenCalledWith('echo "hello world"', {
				timeoutSeconds: 60,
				env: {},
				workingDirectory: './',
			});
	...
	describe('Parameter Validation', () => {
		it('should return error when command is missing', async () => {
			...
			expect(mockChatService.errorLine).toHaveBeenCalledWith('[runShellCommand] command is required');
	...
	describe('Environment Variables', () => {
		it('should pass environment variables correctly', async () => {
			...
			expect(mockFileSystem.executeCommand).toHaveBeenCalledWith('echo $NODE_ENV', {
				timeoutSeconds: 60,
				env,
				workingDirectory: './',
			});
	...
	describe('Error Handling', () => {
		...
		it('should handle network timeouts', async () => {
			...
			const result = await execute({ command: 'sleep 60', timeoutSeconds: 30 }, registry);
	...
	describe('File System Dirty State', () => {
		it('should mark file system as dirty on successful command', async () => {
			const mockResult = { ok: true, exitCode: 0, stdout: 'success', stderr: '' };
			mockFileSystem.executeCommand.mockResolvedValue(mockResult);
		...
		it('should not mark file system as dirty on failed command', async () => {
			const mockResult = { ok: false, exitCode: 1, stdout: '', stderr: 'error' };
			mockFileSystem.executeCommand.mockResolvedValue(mockResult);
		...
		it('should not mark file system as dirty on exception', async () => {
			mockFileSystem.executeCommand.mockRejectedValue(new Error('Network error'));
